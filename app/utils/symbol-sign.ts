...
import { PrivateKey, utils, Hash256, PublicKey } from "symbol-sdk";
 ...
 * トランザクションに連署する
 */
export async function signTransaction(params: SignTransactionParams): Promise<SignResult> {
  ...
  try {
    ...
    // トランザクションハッシュ作成
    const transactionHash = new Hash256(utils.hexToUint8(params.transactionHash));
 ...
 * 公開鍵から署名者のアドレスを取得
 */
export function getAddressFromPublicKey(publicKey: string, network: NetworkType): string {
  try {
    ...
    throw new Error(`アドレス取得に失敗しました: ${error instanceof Error ? error.message : '不明なエラー'}`);
  }
}
...
export function getPublicKeyFromPrivateKey(privateKey: string): string {
  try {
    ...
    throw new Error(`公開鍵取得に失敗しました: ${error instanceof Error ? error.message : '不明なエラー'}`);
  }
}