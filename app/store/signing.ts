...
export const canSignAtom = atom((get) => {
  ...
  return {
    canSign: !!(
      ...
      signingState.status !== "announcing"
    ),
    missingRequirements: [
 ...
 * 署名処理をリセット
 */
export const resetSigningAtom = atom(null, (get, set) => {
 ...
 * エラー状態をクリア
 */
export const clearSigningErrorAtom = atom(null, (get, set) => {
  ...
  async (get, set, params: SignRequestParams) => {
    ...
    try {
      ...
      set(signingStateAtom, {
        ...
        error: null,
        successMessage:
          announceResult.data?.message || "署名とアナウンスが完了しました",
        lastUpdated: new Date(),
      });
    } catch (error) {
      set(signingStateAtom, {
 ...
 */
export const getSigningCompletedEventAtom = atom(
  (get): SigningCompletedEvent | null => {
    const state = get(signingStateAtom);
